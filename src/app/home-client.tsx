...
export default function Home({
  ...
  const selectedItemCoverUrl = useMemo(() => {
    if (!selectedItem) {
      return null;
    }
          ...
          {viewMode === "feed" && selectedItem ? (
            <div className="app-selected-item flex items-center justify-between gap-3 rounded-lg bg-[#f5f5f7] px-5 py-5">
              <div className="flex min-w-0 items-center gap-4">
                {selectedItemCoverUrl ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img
                    src={selectedItemCoverUrl}
                    alt={`Album art for ${selectedItem.title}`}
                    width={64}
                    height={64}
                    className="h-16 w-16 shrink-0 rounded-md object-cover shadow-[0_2px_8px_rgba(0,0,0,0.12)]"
                  />
                ) : null}
            ...
            >
              ...
              {!selectedAlbum ? (
                <>
                  ...
                  <footer className="app-footer mt-5 grid grid-cols-[1fr_auto_1fr] items-center border-t border-[#d2d2d7]/55 pt-2 text-[12px] font-medium text-[#86868b]">
                    ...
                    {log.musicKind === "song" && log.spotifyTrackId ? (
                      <button
                        ...
                        className="-my-1 flex h-6 w-6 items-center justify-center justify-self-center rounded-full text-[#86868b] transition hover:bg-[#f5f5f7] hover:text-[#1d1d1f]"
