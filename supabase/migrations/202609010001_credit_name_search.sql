-- Include names stored inside item credits in the paginated Feed search.
...
as $$
  ...
  matching as (
    ...
    where (
      ...
      or exists (
        select 1
        from jsonb_array_elements(i.credits) as credit(value)
        cross join lateral jsonb_array_elements_text(
          case
            when jsonb_typeof(credit.value -> 'names') = 'array'
              then credit.value -> 'names'
            else '[]'::jsonb
          end
        ) as credit_name(name)
        where lower(credit_name.name) like '%' || lower(trim(p_search)) || '%'
      )
    )
  ),
  page_rows as (
